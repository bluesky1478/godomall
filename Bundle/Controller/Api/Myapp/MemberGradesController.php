 ...
 */
class MemberGradesController extends \Controller\Api\MyappController
{
    /**
     * {@inheritDoc}
     */
        ...
        try {
            switch ($this->method) {
                case Request::METHOD_GET:
                    $grades = $myappApi->getMemberGradeInfo();
                    $this->httpCode = Response::HTTP_OK;
                    if (empty($grades)) {
                        $this->httpCode = Response::HTTP_NO_CONTENT;
                    } else {
                        $this->myappResult = json_encode($grades, JSON_UNESCAPED_UNICODE);
                    }
                    break;
            }
        } catch (Exception $e) {
